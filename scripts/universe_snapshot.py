...
def _fetch_node(s,node):
    ...
    for page in range(1,101):
        page_rows=None; last_error=None
        for attempt in range(5):
            try:
                r=s.get(LIST_URL,params={'node':node,'page':page,'num':100,'sort':'symbol','asc':1},timeout=12)
                r.raise_for_status(); page_rows=r.json() or []; last_error=None; break
            except Exception as e:
                last_error=type(e).__name__
                if attempt<4: time.sleep(0.6*(attempt+1))
        if last_error:
            errors.append({'node':node,'page':page,'error':last_error,'attempts':5}); break
...
def _normalize(raw):
    ...
    for code,item in dedup.items():
        ...
        tradable.append(code); tradable_items.append(item)
...
def _load_previous():
    try:
        ...
        n=int(stats.get('tradable_universe_count') or 0); items=market.get('items') or []
        stamp=stats.get('updated_at') or codes.get('updated_at') or market.get('updated_at')
        when=dt.datetime.fromisoformat(str(stamp).replace('Z','+00:00')) if stamp else None
        age=(dt.datetime.now(dt.timezone.utc)-when).total_seconds()/86400 if when else 9999
        if n>=MIN_TRADABLE and len(codes.get('codes') or [])>=MIN_TRADABLE and len(items)>=MIN_TRADABLE and age<=MAX_FALLBACK_AGE_DAYS:
            return stats,codes,market,round(age,2)
    except Exception:
        pass
    return None
...
def run():
    DATA.mkdir(parents=True,exist_ok=True); s=_session(); raw=[]; source_errors=[]
    for node in ('sh_a','sz_a'):
        node_rows,node_errors=_fetch_node(s,node); raw.extend(node_rows); source_errors.extend(node_errors)
    ...
    if len(tradable)<MIN_TRADABLE:
        ...
        if previous is not None:
            stats,codes,market,age=previous
            stats['fallback_used']=True; stats['fallback_reason']='fresh market-list acquisition below minimum'
            stats['fresh_attempt_at']=updated; stats['fresh_attempt_tradable']=len(tradable)
            stats['fallback_snapshot_age_days']=age; stats['source_errors']=source_errors
            ...
            print(json.dumps({'status':'FALLBACK','fresh_tradable':len(tradable),'verified_tradable':len(codes.get('codes') or []),'snapshot_age_days':age,'source_errors':source_errors},ensure_ascii=False))
            return stats
        raise SystemExit(f'BLOCKED: dynamic tradable universe too small: {len(tradable)}; no verified snapshot within {MAX_FALLBACK_AGE_DAYS} days')

    payload={'updated_at':updated,'source':'Sina Finance Market_Center.getHQNodeData','raw_count':len(raw),'unique_count':len(dedup),'valid_code_count':len(valid),'tradable_universe_count':len(tradable),'excluded':excluded,'source_errors':source_errors,'fallback_used':False,'definition':'动态获取沪深A股列表；排除ST/退、停牌及8/4/68代码板块。','data_quality':'real_dynamic_universe'}
