...
def run():
    ...
    for node in ('sh_a','sz_a'):
        for page in range(1,101):
            ...
            time.sleep(.02)
    dedup={str(x.get('code')):x for x in raw if x.get('code')}; valid=[]; tradable=[]; tradable_items=[]; excluded={'ST_or退':0,'停牌':0,'非目标板块':0,'代码无效':0}
    ...
    DATA.mkdir(parents=True,exist_ok=True); (DATA/'universe_stats.json').write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding='utf-8'); (DATA/'universe_codes.json').write_text(json.dumps({'updated_at':updated,'source':payload['source'],'codes':tradable},ensure_ascii=False,indent=2),encoding='utf-8'); (DATA/'universe_market.json').write_text(json.dumps({'updated_at':updated,'source':payload['source'],'tradable_universe_count':len(tradable),'items':tradable_items},ensure_ascii=False,separators=(',',':')),encoding='utf-8')
