...
from data_sources import yahoo_kline, tencent_kline, tencent_legacy_kline, eastmoney_kline, sina_kline
ROOT=Path(__file__).resolve().parents[1]
SAMPLES=('600519','000001','300750')
PROVIDERS=(('Yahoo Finance Batch',yahoo_kline),('Tencent QFQ',tencent_kline),('Tencent Legacy',tencent_legacy_kline),('Eastmoney QFQ',eastmoney_kline),('Sina',sina_kline))
...
def run():
    ...
    payload={'provider_health':out,'healthy_providers':healthy,'required_minimum':1,'sample_symbols':list(SAMPLES),'status':'PASS' if healthy>=1 else 'BLOCKED','data_quality':'real_multi_symbol_preflight_retry_free_yahoo'}
    ...
    if healthy<1: raise SystemExit('No K-line provider available across free-provider preflight after retries')
    return payload
