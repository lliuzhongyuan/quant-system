...
_retry = Retry(total=4, connect=4, read=4, status=4, backoff_factor=1.0, status_forcelist=(429,500,502,503,504), allowed_methods=frozenset(['GET']))
...
def fetch_kline(code: str, limit=130):
    ...
    try:
        r=SESSION.get(KLINE_URL,params=params,timeout=15); r.raise_for_status()
        data=r.json().get('data') or {}; arr=data.get('klines') or []
        out=[]
        for s in arr:
            p=s.split(',')
            if len(p)<7: continue
...
def calc_signal(stock, rows):
    ...
    atr14=atr(rows,14); rsi14=rsi(closes,14)
    ...
    if blocked:
        ...
        tier='C'; action='—'
    stop=max(0,c-1.5*atr14); target=c+2.5*atr14
    rr=(target-c)/max(0.01,c-stop)
