...
def score_signal(s,rows):
    ...
    quality_base=round(35+trend+.7*position+.7*momentum+.5*basic)
    opportunity_base=round(25+.9*trend+.5*position+.8*momentum+.9*funds)
    quality=max(0,min(100,quality_base)); opportunity=max(0,min(100,opportunity_base))
