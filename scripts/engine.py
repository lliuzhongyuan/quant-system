...
def full_code(code): return f'{code}.{"SH" if exchange(code)=="sh" else "SZ"}'

def fetch_kline(code, limit=180):
    try:
        r = S.get(KLINE_URL, params={'symbol':exchange(code)+str(code),'scale':240,'ma':'5,10,20,60','datalen':limit}, timeout=12)
        a = r.json() or []
...
def fetch_market():
    ...
    for code,x in dedup.items():
        ...
        if not name or any(k in name.upper() for k in ('ST','退','停牌')): continue
        if code.startswith(('8','4','68')): continue
...
def regime(indices, breadth):
    vals=[x['change_pct'] for x in indices if x.get('change_pct') is not None]
    avg=sum(vals)/len(vals) if vals else 0
    adv=breadth.get('advancers',0); dec=breadth.get('decliners',0); total=max(1,adv+dec)
    breadth_score=adv/total*100
    score=max(0,min(100,round(0.55*(50+avg*12)+0.45*breadth_score)))
...
def rsi(a,k=14):
    ...
    g=[max(a[i]-a[i-1],0) for i in range(-k,0)]; l=[max(a[i-1]-a[i],0) for i in range(-k,0)]
    ag=sum(g)/k; al=sum(l)/k
...
def macd(a):
    ...
    # align EMA12 to EMA26's timeline
    dif=[e12[i+14]-e26[i] for i in range(len(e26))]
...
def kdj(rows):
    ...
    for i in range(8,len(rows)):
        hi=max(x['high'] for x in rows[i-8:i+1]); lo=min(x['low'] for x in rows[i-8:i+1])
        rsv=50 if hi==lo else (rows[i]['close']-lo)/(hi-lo)*100
        k=(2*k+rsv)/3; d=(2*d+k)/3
    return k,d,3*k-2*d

def score_signal(s, rows):
    if len(rows)<80 or not s.get('price'): return None
    c=[x['close'] for x in rows]; v=[x['volume'] for x in rows]; p=c[-1]
    m5,m10,m20,m60=[sma(c,k) for k in (5,10,20,60)]; a=atr(rows); rs=rsi(c); dif,dea,mh=macd(c); k,d,j=kdj(rows)
    vr=v[-1]/max(sma(v,20) or 1,1); prev60=min(c[-61:-1]); hi60=max(c[-61:-1]); pos=(p-prev60)/(hi60-prev60) if hi60>prev60 else .5
    ret5=p/c[-6]-1; ret20=p/c[-21]-1; dd20=p/max(c[-21:-1])-1; vol5=sum(v[-5:])/5; vol20=sum(v[-20:])/20; vc=vol5/max(vol20,1)
    ...
    quality=max(0,min(100,round(35+trend+.7*position+.7*momentum+.5*basic-risk)))
    opportunity=max(0,min(100,round(25+.9*trend+.5*position+.8*momentum+.9*funds-risk)))
    ...
    st=[]; why=[]
    def add(k,reason): st.append(k); why.append(reason)
    if p/prev60<=1.35 and p<m20*1.25 and m5>=m10 and vr>=1.15 and ret20>-.05 and risk<20:add('A','60日低位+均线修复+量能启动')
    prior20=max(c[-21:-1])
    ...
    resonance=len([x for x in st if x!='H'])
    tier='D' if blocked else 'S' if resonance>=3 and quality>=70 and opportunity>=70 else 'A' if resonance>=2 and quality>=58 else 'B' if resonance else 'C'
...
def scan_all(workers=12,kline_limit=180):
    stocks=fetch_market(); indices=fetch_indices(); total=len(stocks)
    breadth={'advancers':sum(1 for x in stocks if x['change_pct']>0),'decliners':sum(1 for x in stocks if x['change_pct']<0),'flat':sum(1 for x in stocks if x['change_pct']==0),'limit_up':sum(1 for x in stocks if x['change_pct']>=9.5),'limit_down':sum(1 for x in stocks if x['change_pct']<=-9.5)}
    rg=regime(indices,breadth); now=dt.datetime.now(dt.timezone.utc).isoformat()
    write_json(DATA/'scan_status.json',{'status':'running','started_at':now,'universe':total,'scanned':0,'failed':0,'progress':0})
    results=[]; failed=0
    def one(s):
        rows=fetch_kline(s['symbol'],kline_limit); q=score_signal(s,rows)
        return q,rows
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futures={ex.submit(one,s):s for s in stocks}
        for i,f in enumerate(as_completed(futures),1):
            ...
            if i%100==0 or i==total:
                write_json(DATA/'scan_status.json',{'status':'running','started_at':now,'universe':total,'scanned':i,'failed':failed,'progress':round(i/max(1,total)*100,1)})
    results.sort(key=lambda x:(x[0]['tier']=='D',-x[0]['resonance_count'],-x[0]['opportunity_score'],-x[0]['quality_score']))
    signals=[x[0] for x in results]
    qualified=[x for x in results if x[0]['tier'] in ('S','A','B')]
    # Keep detailed K-lines bounded while signals.json retains the full successfully scanned universe.
    keep=sorted(qualified,key=lambda x:(-x[0]['opportunity_score'],-x[0]['quality_score']))[:600]
    for p in STOCKS.glob('*.json'):
        if p.name not in {x[0]['symbol']+'.json' for x in keep}: p.unlink(missing_ok=True)
    for q,rows in keep:
        write_json(STOCKS/(q['symbol']+'.json'),{'stock':q,'klines':rows})
    market_obj={'updated_at':now,'source':'Sina Finance A-share list + daily K-line','universe':total,'scanned':len(signals),'failed':failed,'indices':indices,'breadth':breadth,'regime':rg,'data_quality':'real_market_and_daily_technical'}
...
def update_news():
    items=[]; source='Sina Finance 7x24'
    try:
        html=S.get(NEWS_URL,timeout=8).text
        # Conservative extraction: only visible anchors with plausible news length.
        for m in re.finditer(r'<a[^>]+href=[\'\"]([^\'\"]+)[\'\"][^>]*>(.*?)</a>',html,re.I|re.S):
            ...
            if 8<=len(title)<=120 and ('finance.sina.com.cn' in href or href.startswith('/')):
                if title not in {x['title'] for x in items}: items.append({'title':title,'url':href if href.startswith('http') else 'https://finance.sina.com.cn'+href,'source':source,'time':dt.datetime.now(dt.timezone.utc).isoformat()})
            if len(items)>=30: break
    except Exception: pass
    write_json(DATA/'news.json',{'updated_at':dt.datetime.now(dt.timezone.utc).isoformat(),'source':source,'items':items,'count':len(items),'data_quality':'verified_web_fetch_only'})
    return items

if __name__=='__main__':
    scan_all(); update_news()
