...
SESSION = requests.Session(); SESSION.headers.update({'User-Agent': UA, 'Referer': 'https://quote.eastmoney.com/'})
...
def atomic_write(path: Path, obj: Any):
    ...
    tmp=path.with_suffix(path.suffix+'.tmp'); tmp.write_text(json.dumps(obj,ensure_ascii=False,separators=(',',':')),encoding='utf-8'); tmp.replace(path)
...
def is_valid_stock(item):
    ...
    if code.startswith(('8','4','68')): return False
...
def fetch_market(page_size=100,max_pages=100):
    ...
    for pn in range(1,max_pages+1):
        ...
        try:
            r=SESSION.get(QUOTE_URL,params=params,timeout=15); r.raise_for_status(); data=r.json().get('data') or {}; diff=data.get('diff') or []
        except Exception as e:
            print('market page failed',pn,e); break
        if not diff: break
        rows.extend(x for x in diff if is_valid_stock(x)); total=int(data.get('total') or 0)
        if total and len(rows)>=total: break
        if len(diff)<page_size: break
        time.sleep(0.08)
    out={str(x['f12']):x for x in rows}; result=[]
...
def fetch_kline(code,limit=130):
    ...
    try:
        r=SESSION.get(KLINE_URL,params=params,timeout=15); r.raise_for_status(); arr=(r.json().get('data') or {}).get('klines') or []; out=[]
        for s in arr:
            p=s.split(',')
            if len(p)>=7: out.append({'date':p[0],'open':num(p[1]),'close':num(p[2]),'high':num(p[3]),'low':num(p[4]),'volume':num(p[5],0),'amount':num(p[6],0)})
        return out
...
def atr(rows,n=14):
    ...
    for i in range(1,len(rows)):
        h,l,pc=rows[i]['high'],rows[i]['low'],rows[i-1]['close'];trs.append(max(h-l,abs(h-pc),abs(l-pc)))
    return sum(trs[-n:])/n if len(trs)>=n else None
def max_prev(vals,n): return max(vals[-n-1:-1]) if len(vals)>=n+1 else None
def min_prev(vals,n): return min(vals[-n:]) if len(vals)>=n else None

def turnover_ok(stock): return (stock.get('turnover') or 0)>0.5

def calc_signal(stock,rows):
    if len(rows)<65 or not stock.get('price'): return None
    closes=[x['close'] for x in rows];vols=[x['volume'] for x in rows];c=closes[-1]
    ma5=sma(closes,5);ma10=sma(closes,10);ma20=sma(closes,20);ma60=sma(closes,60);atr14=atr(rows,14);rsi14=rsi(closes,14)
    if atr14 is None or atr14<=0:return None
    avgv20=sma(vols,20) or 0;vr=vols[-1]/avgv20 if avgv20 else None;prev20=max_prev(closes,20);low60=min_prev(closes,60);high60=max(closes[-60:])
    pos60=(c-low60)/(high60-low60) if high60 and low60 is not None and high60>low60 else .5;slope20=c/closes[-21]-1 if len(closes)>=21 and closes[-21] else 0;ret5=c/closes[-6]-1 if len(closes)>=6 else 0;draw_from_high=c/max(closes[-20:])-1 if len(closes)>=20 else 0
    ...
    if ma5 and ma10 and ma20 and ma5>ma10>ma20 and -.12<=draw_from_high<=-.025 and c>=ma10*.985 and vol_contraction<.9:strat+=['C'];names+=['回踩二波'];reasons.append('主升趋势回踩+缩量+均线承接')
    if ma20 and c>ma20 and ret5>-.06 and (vr or 0)>=1.1 and turnover_ok(stock):strat+=['D'];names+=['筹码结构穿透'];reasons.append('成交成本结构代理+趋势承接')
    if (stock.get('total_mv') or 0)>=100e8 and (stock.get('pe') is None or 0<stock.get('pe')<=60) and c>ma20:strat+=['E'];names+=['龙头强度'];reasons.append('大市值+趋势质量')
    ...
    quality=min(100,q);opportunity=30+len([x for x in strat if x!='H'])*8
    ...
    if blocked:tier='D';action='🔴 风险拦截';quality=min(quality,55);opportunity=min(opportunity,35)
    elif len([x for x in strat if x!='H'])>=3 and quality>=70 and opportunity>=75:tier='S';action='🟢 当前可买' if stock.get('change_pct',0)<=3.5 else '🟡 等待回踩'
    elif len([x for x in strat if x!='H'])>=2 and quality>=60:tier='A';action='🟡 等待确认'
    elif strat:tier='B';action='👀 观察'
    else:tier='C';action='—'
    stop=max(0,c-1.5*atr14);target=c+2.5*atr14;rr=(target-c)/max(.01,c-stop);reason='；'.join(reasons[:4]) or '暂无有效策略信号'
    return {**stock,'ma5':round(ma5,3),'ma10':round(ma10,3),'ma20':round(ma20,3),'ma60':round(ma60,3),'rsi14':round(rsi14,2) if rsi14 is not None else None,'atr14':round(atr14,3),'volume_ratio_calc':round(vr,2) if vr else None,'position60':round(pos60,3),'return5d':round(ret5*100,2),'drawdown20':round(draw_from_high*100,2),'strategy_keys':strat,'strategy_names':names,'resonance_count':len([x for x in strat if x!='H']),'quality_score':round(quality),'opportunity_score':round(opportunity),'tier':tier,'action_status':action,'signal_reason':reason,'target_price':round(target,2),'stop_loss':round(stop,2),'risk_reward':round(rr,2),'signal_time':dt.datetime.now(dt.timezone.utc).isoformat(),'data_quality':'technical_real'}

def fetch_fundamental(code):
    try:
        r=SESSION.get(STOCK_URL,params={'secid':secid(code),'ut':UT,'fields':'f12,f14,f162,f164,f167,f170,f171,f173,f116,f117'},timeout=10);r.raise_for_status();d=r.json().get('data') or {}
...
def scan_all(workers=12,kline_limit=130):
    stocks=fetch_market();atomic_write(DATA/'market.json',{'updated_at':dt.datetime.now(dt.timezone.utc).isoformat(),'count':len(stocks),'source':'Eastmoney clist/get','items':stocks});signals=[]
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if sig:
                signals.append(sig)
                if sig['tier'] in ('S','A'):atomic_write(STOCKS/f"{s['symbol']}.json",{'stock':sig,'klines':rows[-130:],'source':'Eastmoney K-line fqt=1'})
    ...
    payload={'updated_at':dt.datetime.now(dt.timezone.utc).isoformat(),'universe_count':len(stocks),'scanned_count':len(signals),'source':'Eastmoney realtime + daily K-line','strategies':{'A':'低位启动','B':'主升突破','C':'回踩二波','D':'筹码结构穿透','E':'龙头强度','F':'超跌反转','G':'量价异动','H':'风险拦截'},'items':signals[:300]};atomic_write(DATA/'signals.json',payload);return payload
